...
PROJECT_ROOT = _resolve_path("DIANA_API_ROOT", Path(__file__).resolve().parents[2])
LIB_DIR = _resolve_path("DIANA_API_LIB_DIR", PROJECT_ROOT / "lib")
...
scriptLibraryNames = {"Linux": "libDianaApi.so", "Windows": "DianaApi.dll"}

scriptLibraryPath = (LIB_DIR / scriptLibraryNames[platform.system()]).resolve()
if not scriptLibraryPath.exists():
    raise FileNotFoundError(f"无法在 {scriptLibraryPath} 找到底层库，请检查lib目录。")
api_mod = None
if sys.version_info < (3, 8):
    api_mod = CDLL(str(scriptLibraryPath))
else:
    api_mod = CDLL(str(scriptLibraryPath), winmode=0)
...
DIANA_JOINTS_POSITION._fields_ = [("values", c_double * 7)]
...
DIANA_JOINTS_FORCE._fields_ = [("values", c_double * 7)]
...
DIANA_TCP_POSE._fields_ = [("values", c_double * 6)]
...
DIANA_JOINTS_SPEED._fields_ = [("values", c_double * 7)]
...
DIANA_JOINTS_SPEED_L._fields_ = [("values", c_double * 6)]
...
DIANA_JOINTS_ACC._fields_ = [("values", c_double * 2)]
...
DIANA_FRAME_MATRIX._fields_ = [("values", c_double * 16)]
...
DIANA_FORCE_DIRECTION._fields_ = [("values", c_double * 3)]
...
DIANA_JOINT_ANGULAR_VEL._fields_ = [("values", c_double * 7)]
...
DIANA_JOINT_ACC._fields_ = [("values", c_double * 7)]
...
DIANA_JOINT_CURRENT._fields_ = [("values", c_double * 7)]
...
DIANA_JOINT_TORQUE._fields_ = [("values", c_double * 7)]
...
DIANA_TCP_FORCE._fields_ = [("values", c_double * 6)]
...
DIANA_DEFAULT_TCP._fields_ = [("values", c_double * 16)]
...
DIANA_TCP_VECTOR._fields_ = [("values", c_double * 6)]
...
DIANA_RPY_VECTOR._fields_ = [("values", c_double * 3)]
...
DIANA_AXIS_VECTOR._fields_ = [("values", c_double * 3)]
...
DIANA_JOINT_COLLISION._fields_ = [("values", c_double * 7)]
...
DIANA_CART_COLLISION._fields_ = [("values", c_double * 6)]
...
DIANA_JOINT_STIFF._fields_ = [("values", c_double * 7)]
...
DIANA_JOINT_DAMP._fields_ = [("values", c_double * 7)]
...
DIANA_CART_STIFF._fields_ = [("values", c_double * 6)]
...
DIANA_CART_DAMP._fields_ = [("values", c_double * 6)]
...
DIANA_TCP_PAYLOAD._fields_ = [("values", c_double * 10)]
...
DIANA_THRESHOLD_TORQUE._fields_ = [("values", c_double * 7)]
...
DIANA_DH_A._fields_ = [("values", c_double * 7)]
...
DIANA_DH_Alpha._fields_ = [("values", c_double * 7)]
...
DIANA_DH_D._fields_ = [("values", c_double * 7)]
...
DIANA_DH_Theta._fields_ = [("values", c_double * 7)]
...
DIANA_JACOBI_MATRIX._fields_ = [("values", c_double * 42)]
...
DIANA_DIGTAL_VALUE_ALL._fields_ = [("values", c_int * 8)]
...
DIANA_ANALOG_VALUE_ALL._fields_ = [("values", c_double * 2)]
...
DIANA_ANALOG_MODE._fields_ = [("values", c_int * 2)]
...
DIANA_DH_R._fields_ = [("values", c_double * 28)]
...
DIANA_DH_WRT._fields_ = [("values", c_double * 6)]
...
DIANA_DH_TRT._fields_ = [("values", c_double * 3)]
...
DIANA_DH_CONFID._fields_ = [("values", c_double * 2)]
...
DIANA_DH_RTw2b._fields_ = [("values", c_double * 6)]
...
DIANA_DH_RTf2t._fields_ = [("values", c_double * 3)]
...
DIANA_KP._fields_ = [("values", c_double * 7)]
...
DIANA_KD._fields_ = [("values", c_double * 7)]
...
DIANA_ACTIVE_TCP._fields_ = [("values", c_double * 6)]
...
DIANA_GRAV_INFO._fields_ = [("values", c_double * 3)]
...
def moveJToTarget(
    joints, v, a, zv_shaper_order=0, zv_shaper_frequency=0, zv_shaper_damping_ratio=0, ipAddress=""
):
...
def moveJToPose(
    pose, v, a, zv_shaper_order=0, zv_shaper_frequency=0, zv_shaper_damping_ratio=0, ipAddress=""
):
...
def moveJ(
    ...
    ret = moveJToTarget(
        joints, v, a, zv_shaper_order, zv_shaper_frequency, zv_shaper_damping_ratio, ipAddress
    )
    return message(ret)
...
def speedL(speed, acc, t=0.0, ipAddress=""):
    ...
    active_tcp = c_void_p(0)
    api_mod.speedL.argtypes = [
        POINTER(DIANA_JOINTS_SPEED_L),
        POINTER(DIANA_JOINTS_ACC),
        c_double,
        c_void_p,
        c_char_p,
    ]
    ...
    ret = api_mod.speedL(
        dianaJointsSpeed, dianaJointsAcc, t, active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def forward(joints, pose, ipAddress=""):
    ...
    ret = api_mod.forward(
        dianaJointsPosition, byref(dianaTcpPose), active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(dianaTcpPose.values)):
...
def inverse(pose, joints, ipAddress=""):
    ...
    ret = api_mod.inverse(
        dianaTcpPose, byref(dianaJointsPosition), active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(joints)):
...
def getDHCaliResult(rDH, wRT, tRT, confid, ipAddress=""):
    ...
    api_mod.getDHCaliResult.argtypes = [
        POINTER(DIANA_DH_R),
        POINTER(DIANA_DH_WRT),
        POINTER(DIANA_DH_TRT),
        POINTER(DIANA_DH_CONFID),
        c_char_p,
    ]
...
def setDH(a, alpha, d, theta, ipAddress=""):
    ...
    api_mod.setDH.argtypes = [
        POINTER(DIANA_DH_A),
        POINTER(DIANA_DH_Alpha),
        POINTER(DIANA_DH_D),
        POINTER(DIANA_DH_Theta),
        c_char_p,
    ]
    ret = api_mod.setDH(
        byref(arr_aDH),
...
def enterForceMode(
    ...
    api_mod.enterForceMode.argtypes = [
        c_int,
        POINTER(DIANA_FRAME_MATRIX),
        POINTER(DIANA_FORCE_DIRECTION),
        c_double,
        c_double,
        c_double,
        c_char_p,
    ...
    ret = api_mod.enterForceMode(
        frame_type,
        byref(dianaFrameMatrix),
        byref(dianaForceDirection),
        force_value,
        max_approach_velocity,
        max_allow_tcp_offset,
        bytes(ipAddress.encode("utf-8")),
...
def leaveForceMode(mode, ipAddress=""):
    if not isinstance(mode, mode_e):
        raise TypeError("mode must be an instance of mode_e Enum")
...
def addMoveL(id_path, joints, vel, acc, blendradius, ipAddress=""):
    ...
    ret = api_mod.addMoveL(
        id_path, dianaJointPos, vel, acc, blendradius, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def homogeneous2Pose(matrix, pose):
    ...
    api_mod.homogeneous2Pose.argtypes = [POINTER(DIANA_FRAME_MATRIX), POINTER(DIANA_TCP_POSE)]
...
def pose2Homogeneous(pose, matrix):
    ...
    _matrix = DIANA_FRAME_MATRIX()
    _matrix.values = (c_double * 16)()
    api_mod.pose2Homogeneous.argtypes = [POINTER(DIANA_TCP_POSE), POINTER(DIANA_FRAME_MATRIX)]
...
def servoL(tcp_pose, t=0.01, ah_t=0.01, gain=300, scale=1000, ipAddress=""):
    ...
    ret = api_mod.servoL(
        dianaTcpPose, t, ah_t, gain, scale, active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def servoJ_ex(joints_pos, t=0.01, ah_t=0.01, gain=300, realiable=False, ipAddress=""):
    ...
    ret = api_mod.servoJ_ex(
        dianaJointsPosition, t, ah_t, gain, realiable, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def servoL_ex(tcp_pose, t=0.01, ah_t=0.01, gain=300, scale=1000, realiable=False, ipAddress=""):
    ...
    ret = api_mod.servoL_ex(
        dianaTcpPose, t, ah_t, gain, scale, realiable, active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def speedJ_ex(speed, acc, t=0.0, realiable=False, ipAddress=""):
    ...
    for index in range(0, len(speed)):
        dianaJointsSpeed.values[index] = speed[index]
    api_mod.speedJ_ex.argtypes = [POINTER(DIANA_JOINTS_SPEED), c_double, c_double, c_bool, c_char_p]
...
def speedL_ex(speed, acc, t=0.0, realiable=False, ipAddress=""):
    ...
    ret = api_mod.speedL_ex(
        dianaJointsSpeed, dianaJointsAcc, t, realiable, active_tcp, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def inverse_ext(ref_joints, pose, joints, ipAddress=""):
    ...
    ret = api_mod.inverse_ext(
        refJointsPosition,
        dianaTcpPose,
        byref(dianaJointsPosition),
        active_tcp,
        bytes(ipAddress.encode("utf-8")),
    )
    for index in range(0, len(joints)):
...
def createComplexPath(path_type, ipAddress=""):
    ...
    ret = api_mod.createComplexPath(
        path_type.value, byref(complex_path_id), bytes(ipAddress.encode("utf-8"))
    )
    return ret, complex_path_id


def addMoveLSegmentByTarget(
    complex_path_id,
    joints,
    vel,
    acc,
    blendradius,
    zv_shaper_order=0,
    zv_shaper_frequency=0,
    zv_shaper_damping_ratio=0,
    ipAddress="",
):
    dianaJointPos = DIANA_JOINTS_POSITION()
    dianaJointPos.values = (c_double * 7)()
    for index in range(0, len(joints)):
        dianaJointPos.values[index] = joints[index]
    api_mod.addMoveLByTarget.argtypes = [
        c_uint,
        POINTER(DIANA_JOINTS_POSITION),
        c_double,
        c_double,
...
def addMoveLSegmentByPose(
    ...
    for index in range(0, len(dianaTcpPose.values)):
        dianaTcpPose.values[index] = pose[index]
    api_mod.addMoveLByPose.argtypes = [
        c_uint,
        POINTER(DIANA_TCP_POSE),
        c_double,
        c_double,
...
def addMoveJSegmentByTarget(
    ...
    for index in range(0, len(joints)):
        dianaJointPos.values[index] = joints[index]
    api_mod.addMoveJByTarget.argtypes = [
        c_uint,
        POINTER(DIANA_JOINTS_POSITION),
        c_double,
        c_double,
...
def addMoveJSegmentByPose(
    ...
    for index in range(0, len(dianaTcpPose.values)):
        dianaTcpPose.values[index] = pose[index]
    api_mod.addMoveJByPose.argtypes = [
        c_uint,
        POINTER(DIANA_TCP_POSE),
        c_double,
        c_double,
...
def addMoveCSegmentByTarget(
    ...
    api_mod.addMoveCByTarget.argtypes = [
        c_uint,
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_double,
...
def addMoveCSegmentByPose(
    ...
    api_mod.addMoveCByPose.argtypes = [
        c_uint,
        POINTER(DIANA_TCP_POSE),
        POINTER(DIANA_TCP_POSE),
        c_double,
...
def destroyComplexPath(complex_path_id, ipAddress=""):
    ...
    ret = api_mod.destroyComplexPath(
        complex_path_id, bytes(ipAddress.encode("utf-8")), bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def enterForceMode_ex(
    ...
    if active_tcp == None:
        ...
        api_mod.enterForceMode_ex.argtypes = [
            POINTER(DIANA_FORCE_DIRECTION),
            c_double,
            c_double,
            c_double,
        ...
        ret = api_mod.enterForceMode_ex(
            byref(arr_ForceDirection),
            forceValue,
            maxApproachVelocity,
            maxAllowTcpOffset,
            use_active_tcp,
...
def readDI(group_name, di_name, ipAddress=""):
    ...
    ret = api_mod.readDI(
        bytes(group_name, encoding="utf-8"),
        bytes(di_name, encoding="utf-8"),
        byref(DIValue),
        bytes(ipAddress.encode("utf-8")),
    )
    return ret, DIValue
...
def readAO(group_name, do_name, ipAddress=""):
    AOValue = c_double()
    AOMode = c_int()
    api_mod.readAO.argtypes = [c_char_p, c_char_p, POINTER(c_int), POINTER(c_double), c_char_p]
    api_mod.readAO.restype = c_int
    ret = api_mod.readAO(
        bytes(group_name, encoding="utf-8"),
        bytes(do_name, encoding="utf-8"),
        byref(AOMode),
        byref(AOValue),
        bytes(ipAddress.encode("utf-8")),
    )
...
def getWayPoint(waypointName, dblTcppos, dblJoints, ipAddress=""):
    ...
    ret = api_mod.getWayPoint(
        bytes(waypointName.encode("utf-8")),
        dianaTcpPose,
        dianaJointPos,
        bytes(ipAddress.encode("utf-8")),
    )
...
def getDefaultActiveTcp(default_tcp, ipAddress=""):
    defaultTcp = DIANA_DEFAULT_TCP()
    defaultTcp.values = (c_double * 16)()
    api_mod.getDefaultActiveTcp.argtypes = [POINTER(DIANA_DEFAULT_TCP), c_char_p]
    api_mod.getDefaultActiveTcp.restype = c_int
    ret = api_mod.getDefaultActiveTcp(defaultTcp, bytes(ipAddress.encode("utf-8")))
    for index in range(0, len(defaultTcp.values)):
...
def getDefaultActiveTcpPose(arrPose, ipAddress=""):
    ...
    api_mod.getDefaultActiveTcpPose.argtypes = [POINTER(DIANA_TCP_POSE), c_char_p]
    api_mod.getDefaultActiveTcpPose.restype = c_int
    ret = api_mod.getDefaultActiveTcpPose(pose, bytes(ipAddress.encode("utf-8")))
...
def getActiveTcpPayload(payload, ipAddress=""):
    tcpPayload = DIANA_TCP_PAYLOAD()
    tcpPayload.values = (c_double * 10)()
    api_mod.getActiveTcpPayload.argtypes = [POINTER(DIANA_TCP_PAYLOAD), c_char_p]
    api_mod.getActiveTcpPayload.restype = c_int
    ret = api_mod.getActiveTcpPayload(tcpPayload, bytes(ipAddress.encode("utf-8")))
    for index in range(0, len(tcpPayload.values)):
...
def zeroSpaceManualMove(direction, dblJointsVel, dblJointAcc, ipAddress=""):
    if not isinstance(direction, zero_space_move_direction):
        raise TypeError("direction must be an instance of zero_space_move_direction Enum")
    ...
    api_mod.zeroSpaceManualMove.argtypes = [
        c_int,
        POINTER(DIANA_JOINT_ANGULAR_VEL),
        POINTER(DIANA_JOINT_ACC),
        c_char_p,
    ]
...
def moveTcp_ex(coordinate, direction, velocity, accleartion, ipAddress=""):
    if not isinstance(coordinate, coordinate_e):
        raise TypeError("coordinate must be an instance of coordinate_e Enum")
    if not isinstance(direction, tcp_direction_e):
        raise TypeError("direction must be an instance of tcp_direction_e Enum")
...
def setExternalAppendTorCutoffFreq(dblFreq, ipAddress=""):
    ...
    ret = api_mod.setExternalAppendTorCutoffFreq(dblFreq, bytes(ipAddress.encode("utf-8")))
    return message(ret)
...
def poseTransform(srcPose, srcMatrixPose, dstMartixPose, dstPose):
    ...
    for index in range(0, len(dstTarPose.values)):
        dstPose[index] = -1
    api_mod.poseTransform.argtypes = [
        POINTER(DIANA_TCP_POSE),
        POINTER(DIANA_TCP_POSE),
        POINTER(DIANA_TCP_POSE),
        POINTER(DIANA_TCP_POSE),
    ...
    ret = api_mod.poseTransform(
        byref(srcTcpPose), byref(srcMatrix), byref(dstTcpPose), byref(dstTarPose)
    )
    for index in range(0, len(dstTarPose.values)):
...
def inverseClosedFull(
    pose, lock_joint_index, lock_joint_position, ref_joints, active_tcp=None, ipAddress=""
):
    refJointsPosition = DIANA_JOINTS_POSITION()
    refJointsPosition.values = (c_double * 7)()
    for index in range(0, len(ref_joints)):
    ...
    if active_tcp == None:
        use_active_tcp = c_void_p(0)
        api_mod.inverseClosedFull.argtypes = [
            POINTER(DIANA_TCP_POSE),
            c_int,
            c_double,
            POINTER(DIANA_JOINTS_POSITION),
        ...
        id = api_mod.inverseClosedFull(
            byref(dianaPose),
            lock_joint_index,
            lock_joint_position,
            byref(refJointsPosition),
            use_active_tcp,
            bytes(ipAddress.encode("utf-8")),
        )
    else:
...
def getInverseClosedJoints(id, index, joints, ipAddress=""):
    ...
    api_mod.getInverseClosedJoints.argtypes = [
        c_int,
        c_int,
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
...
def getTcpForceInToolCoordinate(forces, ipAddress=""):
    ...
    ret = api_mod.getTcpForceInToolCoordinate(
        byref(dianaTcpForce), bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(dianaTcpForce.values)):
...
def calculateJacobi(matrixJacobi, joints, ipAddress=""):
    ...
    api_mod.calculateJacobi.argtypes = [
        POINTER(DIANA_JACOBI_MATRIX),
        POINTER(DIANA_JOINTS_POSITION),
        c_int,
        c_char_p,
    ]
    api_mod.calculateJacobi.restype = c_int
    ret = api_mod.calculateJacobi(
        byref(dianaJacobiMatrix), dianaJoints, _joint_count, bytes(ipAddress.encode("utf-8"))
    )
    if ret < 0:
        return message(ret)
...
def calculateJacobiTF(matrixJacobi, joints, toolMatrix, ipAddress=""):
    ...
    dianaJacobiMatrix = DIANA_JACOBI_MATRIX()
    dianaJacobiMatrix.values = (c_double * 42)()
    api_mod.calculateJacobiTF.argtypes = [
        POINTER(DIANA_JACOBI_MATRIX),
        POINTER(DIANA_JOINTS_POSITION),
        c_int,
        POINTER(DIANA_ACTIVE_TCP),
    ...
    ret = api_mod.calculateJacobiTF(
        byref(dianaJacobiMatrix),
        dianaJoints,
        _joint_count,
        dianaToolMatrix,
        bytes(ipAddress.encode("utf-8")),
...
def setControllerFeature(feature_code, enable, ipAddress=""):
    ...
    ret = api_mod.setControllerFeatureCode(
        feature_code.value, enable, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def testControllerFeature(feature_code, ipAddress=""):
    ...
    ret = api_mod.testControllerFeature(
        feature_code.value, byref(enable), bytes(ipAddress.encode("utf-8"))
    )
    return enable
...
def getMechanicalJointsPositionRange(arrMin, arrMax, ipAddress=""):
    ...
    api_mod.getMechanicalJointsPositionRange.argtypes = [
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
    ]
    api_mod.getMechanicalJointsPositionRange.restype = c_int
    ret = api_mod.getMechanicalJointsPositionRange(
        byref(jointsMin), byref(jointsMax), bytes(ipAddress.encode("utf-8"))
    )
...
def getMechanicalMaxCartVelAcc(ipAddress=""):
    ...
    api_mod.getMechanicalMaxCartVelAcc.argtypes = [
        POINTER(c_double),
        POINTER(c_double),
        POINTER(c_double),
        POINTER(c_double),
    ...
    ret = api_mod.getMechanicalMaxCartVelAcc(
        byref(cartTranslationVel),
        byref(cartRotationVel),
        byref(cartTranslationAcc),
        byref(cartRotationAcc),
        bytes(ipAddress.encode("utf-8")),
    )
    return (
        message(ret),
        cartTranslationVel.value,
        cartRotationVel.value,
        cartTranslationAcc.value,
        cartRotationAcc.value,
    )
...
def getJointsPositionRange(arrMin, arrMax, ipAddress=""):
    ...
    jointsMax = DIANA_JOINTS_POSITION()
    jointsMax.values = (c_double * 7)()
    api_mod.getJointsPositionRange.argtypes = [
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
    ]
    api_mod.getJointsPositionRange.restype = c_int
    ret = api_mod.getJointsPositionRange(
        byref(jointsMin), byref(jointsMax), bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(arrMin)):
        arrMin[index] = jointsMin.values[index]
...
def getMaxCartTranslationVel(ipAddress=""):
    ...
    ret = api_mod.getMaxCartTranslationVel(
        byref(cartTranslationVel), bytes(ipAddress.encode("utf-8"))
    )
...
def getMaxCartTranslationAcc(ipAddress=""):
    ...
    ret = api_mod.getMaxCartTranslationAcc(
        byref(cartTranslationAcc), bytes(ipAddress.encode("utf-8"))
    )
...
def setJointsPositionRange(arrMinPos, arrMaxPos, ipAddress=""):
    ...
    api_mod.setJointsPositionRange.argtypes = [
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
    ]
...
def getJointsSoftLimitRange(arrMin, arrMax, ipAddress=""):
    ...
    jointsMax = DIANA_JOINTS_POSITION()
    jointsMax.values = (c_double * 7)()
    api_mod.getJointsSoftLimitRange.argtypes = [
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
    ]
    api_mod.getJointsSoftLimitRange.restype = c_int
    ret = api_mod.getJointsSoftLimitRange(
        byref(jointsMin), byref(jointsMax), bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(arrMin)):
        arrMin[index] = jointsMin.values[index]
...
def setJointsSoftLimitRange(arrMinPos, arrMaxPos, ipAddress=""):
    ...
    for index in range(0, len(arrMaxPos)):
        jointMax.values[index] = arrMaxPos[index]
    api_mod.setJointsSoftLimitRange.argtypes = [
        POINTER(DIANA_JOINTS_POSITION),
        POINTER(DIANA_JOINTS_POSITION),
        c_char_p,
    ]
...
def getFunctionOptI4(funtion_index: function_index_e, opt_name: function_opt_name_e, ipAddress=""):
    ...
    ret = api_mod.getFunctionOptI4(
        funtion_index.value, opt_name.value, byref(opt_value), bytes(ipAddress.encode("utf-8"))
    )
    if ret == 0:
...
def setFunctionOptI4(
    funtion_index: function_index_e, opt_name: function_opt_name_e, opt_value, ipAddress=""
):
    api_mod.setFunctionOptI4.argtypes = [c_int, c_int, c_int, c_char_p]
    api_mod.setFunctionOptI4.restype = c_int
    ret = api_mod.setFunctionOptI4(
        funtion_index.value, opt_name.value, opt_value, bytes(ipAddress.encode("utf-8"))
    )
    if ret == 0:
        return True
    else:
...
def getJointLockedInCartImpedanceMode(ipAddress=""):
    ...
    ret = api_mod.getJointLockedInCartImpedanceMode(
        byref(isLocked), bytes(ipAddress.encode("utf-8"))
    )
...
def setJointLockedInCartImpedanceMode(islocked, lockIndex=2, ipAddress=""):
    ...
    ret = api_mod.setJointLockedInCartImpedanceMode(
        islocked, lockIndex, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def setJointImpeda(arrStiff, arrDampRatio, ipAddress=""):
    ...
    for index in range(0, len(arrStiff)):
        jointStiff.values[index] = arrStiff[index]
    api_mod.setJointImpeda.argtypes = [POINTER(DIANA_JOINT_STIFF), c_double, c_char_p]
    api_mod.setJointImpeda.restype = c_int
    ret = api_mod.setJointImpeda(jointStiff, arrDampRatio[0], bytes(ipAddress.encode("utf-8")))
    return message(ret)
...
def getJointImpeda(arrStiff, arrDampRatio, ipAddress=""):
    ...
    api_mod.getJointImpeda.argtypes = [POINTER(DIANA_JOINT_STIFF), POINTER(c_double), c_char_p]
    api_mod.getJointImpeda.restype = c_int
    ret = api_mod.getJointImpeda(
        byref(jointStiff), byref(jointDampRatio), bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(arrStiff)):
        arrStiff[index] = jointStiff.values[index]
...
def setCartImpeda(arrStiff, arrDampRatio, ipAddress=""):
    ...
    for index in range(0, len(cartStiff.values)):
        cartStiff.values[index] = arrStiff[index]
    api_mod.setCartImpeda.argtypes = [POINTER(DIANA_CART_STIFF), c_double, c_char_p]
    api_mod.setCartImpeda.restype = c_int
    ret = api_mod.setCartImpeda(cartStiff, arrDampRatio[0], bytes(ipAddress.encode("utf-8")))
    return message(ret)
...
def getCartImpeda(arrStiff, arrDampRatio, ipAddress=""):
    ...
    api_mod.getCartImpeda.argtypes = [POINTER(DIANA_CART_STIFF), POINTER(c_double), c_char_p]
    api_mod.getCartImpeda.restype = c_int
    ret = api_mod.getCartImpeda(
        byref(cartStiff), byref(cartDampRatio), bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(cartStiff.values)):
        arrStiff[index] = cartStiff.values[index]
...
def setThresholdTorque(arrthreshold, ipAddress=""):
    ...
    api_mod.setThresholdTorque.argtypes = [POINTER(DIANA_THRESHOLD_TORQUE), c_char_p]
    api_mod.setThresholdTorque.restype = c_int
    ret = api_mod.setThresholdTorque(threshold, bytes(ipAddress.encode("utf-8")))
    return message(ret)
...
def getThresholdTorque(arrthreshold, ipAddress=""):
    threshold = DIANA_THRESHOLD_TORQUE()
    threshold.values = (c_double * 7)()
    api_mod.getThresholdTorque.argtypes = [POINTER(DIANA_THRESHOLD_TORQUE), c_char_p]
...
def setHeartbeatParam(disconnectTimeout, stopRobotTimeout, ipAddress=""):
    ...
    ret = api_mod.setHeartbeatParam(
        disconnectTimeout, stopRobotTimeout, bytes(ipAddress.encode("utf-8"))
    )
    return ret
...
def getHeartbeatParam(disconnectTimeout, stopRobotTimeout, ipAddress=""):
    ...
    ret = api_mod.getHeartbeatParam(
        byref(disconnectTimeout), byref(stopRobotTimeout), bytes(ipAddress.encode("utf-8"))
    )
    return ret
...
def getTcpPosByTcpName(tcpName, arrPose, ipAddress=""):
    ...
    ret = api_mod.getTcpPoseByTcpName(
        bytes(tcpName.encode("utf-8")), pose, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(pose.values)):
        arrPose[index] = pose.values[index]
    return message(ret)
...
def getTcpPoseByWorkPieceName(workPieceName, arrPose, ipAddress=""):
    ...
    ret = api_mod.getTcpPoseByWorkPieceName(
        bytes(workPieceName.encode("utf-8")), pose, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(pose.values)):
        arrPose[index] = pose.values[index]
    return message(ret)
...
def getPayLoadByTcpName(tcpName, payload, ipAddress=""):
    ...
    ret = api_mod.getPayLoadByTcpName(
        bytes(tcpName.encode("utf-8")), tcpPayload, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(tcpPayload.values)):
        payload[index] = tcpPayload.values[index]
    return message(ret)
...
def setDefaultToolTcpCoordinate(tcpName, ipAddress=""):
    ...
    ret = api_mod.setDefaultToolTcpCoordinate(
        bytes(tcpName.encode("utf-8")), bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def setDefaultWorkPieceTcpCoordinate(workPieceName, ipAddress=""):
    ...
    ret = api_mod.setDefaultWorkPieceTcpCoordinate(
        bytes(workPieceName.encode("utf-8")), bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
