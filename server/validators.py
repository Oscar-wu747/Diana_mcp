...
def _validate_numeric(value: Union[int, float], name: str, min_val: Optional[float] = None, 
...
def validate_joints(joints: Optional[List], param_name: str = "joints") -> List[float]:
    """验证关节位置参数
    ...
    if not isinstance(joints, list):
        raise ValueError(f"{param_name} 必须是列表类型，当前类型: {type(joints).__name__}")
...
def validate_pose(pose: Optional[List], param_name: str = "pose") -> List[float]:
    ...
    # 检查长度
    if len(pose) != 6:
        raise ValueError(f"{param_name} 必须包含 6 个值 [x, y, z, rx, ry, rz]，当前数量: {len(pose)}")
    ...
    for i, value in enumerate(pose):
        ...
        try:
            ...
            raise ValueError(f"姿态值 {i+1} ({'xyz'[i] if i < 3 else 'rxryrz'[i-3]}) 验证失败: {str(e)}")
...
def validate_velocity(velocity: Union[int, float], param_name: str = "velocity") -> float:
    ...
    return _validate_numeric(
        velocity,
        param_name,
        min_val=DEFAULT_VELOCITY_MIN,
        max_val=DEFAULT_VELOCITY_MAX
    )


def validate_acceleration(acceleration: Union[int, float], param_name: str = "acceleration") -> float:
    """验证加速度参数
    ...
    return _validate_numeric(
        acceleration,
        param_name,
        min_val=DEFAULT_ACCELERATION_MIN,
        max_val=DEFAULT_ACCELERATION_MAX
    )
